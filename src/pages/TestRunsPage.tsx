...
type Props = { initialExecutionId?: string; defects: DefectsState; onCreateDefect: (executionId: string) => void; onViewDefect: (id: string) => void; onLinkDefect: (executionId: string, defectId: string) => string | null; projectId: string; userId?: number; data: TestRunsState; cases: TestCase[]; plans: TestPlan[]; areas: ProjectArea[]; types: TestCaseDictionaryValue[]; onChange: Dispatch<SetStateAction<TestRunsState>> }
export function TestRunsPage({ projectId, userId, data, cases, plans, areas, types, onChange, initialExecutionId, defects, onCreateDefect, onViewDefect, onLinkDefect }: Props) {
  const [creating, setCreating] = useState(false), [runId, setRunId] = useState(() => data.executions.find(item => item.projectId === projectId && item.id === initialExecutionId)?.runId ?? ''), [executionId, setExecutionId] = useState(initialExecutionId ?? '')
  const [draft, setDraft] = useState<ExecutionInput | null>(null), [confirming, setConfirming] = useState(false)
  ...
  function create(input: RunInput) {
    try {
      const created = createTestRun(projectId, input, cases, plans, areas, types)
  ...
  return <main className="smoke-app"><header className="page-heading"><h1>Test Runs</h1></header>
    {creating ? <TestRunCreate cases={cases.filter(item => item.projectId === projectId)} plans={plans.filter(item => item.projectId === projectId)} areas={areas.filter(item => item.projectId === projectId)} types={types.filter(item => item.projectId === projectId)} onCreate={create} onCancel={() => setCreating(false)} /> : !run ? <div className="tc-list"><div className="tc-toolbar"><Button className="tc-add" onClick={() => setCreating(true)}>New Test Run</Button></div>
      <table className="tc-table run-list"><thead><tr>{['Name', 'Test Plan', 'Environment / Середовище', 'Build / Збірка', 'Status', 'Progress', 'Started', 'Updated'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{runs.map(item => { const progress = runCounts(data.executions.filter(execution => execution.projectId === projectId && execution.runId === item.id)); return <tr key={item.id} onClick={() => { setRunId(item.id); setExecutionId(''); setConfirming(false) }}><td><button className="tc-open" onClick={() => { setRunId(item.id); setExecutionId(''); setConfirming(false) }}>{item.name}</button></td><td>{item.testPlanTitleSnapshot || '—'}</td><td>{item.environment || '—'}</td><td>{item.build || '—'}</td><td>{item.status}</td><td>{progress.done} / {progress.total}</td><td>{date(item.startedAt)}</td><td>{date(item.updatedAt)}</td></tr> })}</tbody></table>
      {!runs.length && <p className="muted">Запусків поки немає.</p>}
    </div> : <>
      <div className="tc-toolbar"><AccountBackButton disabled={!!draft} onClick={() => { setRunId(''); setExecutionId(''); setConfirming(false) }}>← Test Runs</AccountBackButton><h2>{run.name}</h2><span>{run.status}</span>
      ...
      <dl className="run-metadata">{[['Test Plan', run.testPlanTitleSnapshot], ['Environment / Середовище', run.environment], ['Build / Збірка', run.build], ['Browser', run.browser], ['Device / OS', run.deviceOrOs], ['Started', date(run.startedAt)], ['Completed', date(run.completedAt)]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value || '—'}</dd></div>)}</dl>
