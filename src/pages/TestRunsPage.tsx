...
import { changeRunStatus, createTestRun, runCounts, saveExecution, executionResults, type ExecutionInput, type RunInput } from '@/lib/testRuns'
...
import type { DefectsState, ProjectArea, TestCase, TestCaseDictionaryValue, TestExecution, TestPlan, TestRun, TestRunsState } from '@/types'
import './TestRunsPage.css'
const date = (value?: string | null) => value ? new Date(value).toLocaleString() : '—'
type Props = { setup?: ProjectSetupState; suites?: TestSuitesState; initialRunId?: string; initialCreateSuiteId?: string; onViewSuite?: (id: string) => void; initialExecutionId?: string; defects: DefectsState; onCreateDefect: (executionId: string) => void; onViewDefect: (id: string) => void; onLinkDefect: (executionId: string, defectId: string) => string | null | Promise<string | null>; projectId: string; userId?: number; data: TestRunsState; cases: TestCase[]; plans: TestPlan[]; areas: ProjectArea[]; types: TestCaseDictionaryValue[]; onCreateRun?: (input: RunInput) => Promise<TestRun>; onRunStatus?: (id: string, status: TestRun['status']) => Promise<void>; onExecutionSave?: (id: string, input: ExecutionInput) => Promise<TestExecution>; onChange?: Dispatch<SetStateAction<TestRunsState>> }
export function TestRunsPage({ setup = emptyProjectSetup, suites = { suites: [], links: [] }, initialRunId, initialCreateSuiteId, onViewSuite, projectId, userId, data, cases, plans, areas, types, onCreateRun, onRunStatus, onExecutionSave, onChange, initialExecutionId, defects, onCreateDefect, onViewDefect, onLinkDefect }: Props) {
  ...
  const [draft, setDraft] = useState<ExecutionInput | null>(null), [confirming, setConfirming] = useState(false), [reportOpen, setReportOpen] = useState(false), [error, setError] = useState('')
  ...
  async function create(input: RunInput) {
    try {
      let created: TestRun
      if (onCreateRun) created = await onCreateRun(input)
      else { const state = createTestRun(projectId, input, cases, plans, areas, types, suites.suites, setup); created = state.runs[0]; onChange?.(current => ({ runs: [...current.runs, ...state.runs], executions: [...current.executions, ...state.executions] })) }
      setCreating(false); setRunId(created.id); setExecutionId(''); return null
    }
    catch (cause) { return cause instanceof Error ? cause.message : 'Не вдалося створити запуск.' }
  }
  function changeStatus(status: 'In Progress' | 'Completed') { if (onRunStatus) return onRunStatus(runId, status); if (onChange) onChange(current => changeRunStatus(current, projectId, runId, status)); return Promise.resolve() }
  function complete() { void changeStatus('Completed').then(() => setConfirming(false)).catch(cause => setError(cause instanceof Error ? cause.message : 'Не вдалося завершити запуск.')) }
  function select(id: string) { if (!draft) { setExecutionId(id); setConfirming(false) } }
  function linkedDefects(id: string) { return defects.items.filter(item => item.projectId === projectId && defects.links.some(link => link.projectId === projectId && link.sourceType === 'testExecution' && link.sourceId === id && link.defectId === item.id)) }
  ...
  return <main className="smoke-app"><header className="page-heading"><h1>Test Runs</h1></header>
    {creating ? <TestRunCreate projectId={projectId} setup={setup} initial={createInput} cases={cases.filter(item => item.projectId === projectId)} plans={plans.filter(item => item.projectId === projectId)} areas={areas.filter(item => item.projectId === projectId)} types={types.filter(item => item.projectId === projectId)} onCreate={create} onCancel={() => setCreating(false)} /> : !run ? <div className="tc-list"><div className="tc-toolbar"><AddEntityButton entity="test run" onClick={() => { setCreateInput(undefined); setCreating(true) }} /></div>
      ...
      <div className="tc-toolbar"><AccountBackButton disabled={!!draft} onClick={() => { setRunId(''); setExecutionId(''); setConfirming(false); setReportOpen(false) }}>← Test Runs</AccountBackButton><h2>{run.name}</h2><span>{run.status}</span>
        ...
        {!readOnly && <>{run.status === 'Draft' && <Button aria-label="Start Run" variant="outline" disabled={!!draft} onClick={() => void changeStatus('In Progress').catch(cause => setError(cause instanceof Error ? cause.message : 'Не вдалося почати запуск.'))}>Start Run / Почати запуск</Button>}<Button aria-label="Complete Run" disabled={!!draft} onClick={() => stats.counts['Not Run'] ? setConfirming(true) : complete()}>Complete Run / Завершити запуск</Button></>}
      </div>
      ...
      <div className="tc-toolbar" aria-label="Run progress"><strong>Progress: {stats.done} / {stats.total}</strong>{executionResults.map(result => <ResultBadge key={result} value={result} count={stats.counts[result]} />)}</div>
      ...
      <div className={`tc-layout ${execution ? 'tc-with-panel' : ''}`}><div className="tc-list"><table className="tc-table action-table"><thead><tr>{['Test Case', 'Title', 'Area', 'Priority', 'Result', 'Executed at', 'Actions'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{entries.map(item => { const related = linkedDefects(item.id); return <tr key={item.id} className={executionId === item.id ? 'tc-selected' : ''} onClick={() => select(item.id)}><td><button className="tc-open" disabled={!!draft} onClick={() => select(item.id)}>{item.testCaseSnapshot.code}</button></td><td>{item.testCaseSnapshot.title}</td><td>{item.testCaseSnapshot.areaName || '—'}</td><td>{priorityLabel(item.testCaseSnapshot.priority)}</td><td><ResultBadge value={item.result} /></td><td>{date(item.executedAt)}</td><td><div className="table-row-actions" onClick={event => event.stopPropagation()}><Button type="button" variant="outline" size="sm" disabled={!!draft} aria-label={`${readOnly ? 'View' : item.result === 'Not Run' ? 'Run' : 'Open'} execution ${item.testCaseSnapshot.code}`} onClick={() => select(item.id)}>{readOnly ? 'View / Переглянути' : item.result === 'Not Run' ? 'Run / Виконати' : 'Open / Відкрити'}</Button>{(item.result === 'Fail' || related.length > 0) && <DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="ghost" size="icon" disabled={!!draft} aria-label={`Execution actions ${item.testCaseSnapshot.code}`}><MoreHorizontal /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">{item.result === 'Fail' && <DropdownMenuItem onSelect={() => onCreateDefect(item.id)}>Create Defect / Створити дефект</DropdownMenuItem>}{related.map(defect => <DropdownMenuItem key={defect.id} onSelect={() => onViewDefect(defect.id)}>View {defect.code} / Переглянути</DropdownMenuItem>)}</DropdownMenuContent></DropdownMenu>}</div></td></tr> })}</tbody></table></div>
        {execution && <TestExecutionPanel evidence={<OwnerEvidence key={'evidence-' + execution.id} owner={{ projectId, ownerType: 'testExecution', ownerId: execution.id }} />} defects={defects.items.filter(item => item.projectId === projectId)} linkedDefects={defects.items.filter(item => item.projectId === projectId && defects.links.some(link => link.projectId === projectId && link.sourceType === 'testExecution' && link.sourceId === execution.id && link.defectId === item.id))} onCreateDefect={() => onCreateDefect(execution.id)} onViewDefect={onViewDefect} onLinkDefect={id => onLinkDefect(execution.id, id)} execution={execution} draft={draft} readOnly={readOnly} onChange={input => { if (!readOnly) setDraft(input) }} onSave={() => { if (!readOnly) { const input = draft ?? execution; if (onExecutionSave) void onExecutionSave(execution.id, input).then(() => { setDraft(null); setError('') }).catch(cause => setError(cause instanceof Error ? cause.message : 'Не вдалося зберегти результат.')); else if (onChange) { onChange(current => saveExecution(current, projectId, execution.id, input, userId)); setDraft(null) } } }} onCancel={() => setDraft(null)} onClose={() => setExecutionId('')} first={index === 0} last={index === entries.length - 1} onPrevious={() => select(entries[index - 1].id)} onNext={() => select(entries[index + 1].id)} />}
      </div>
    </>}
  </main>
}
