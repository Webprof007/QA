...
function createRun() { click('Test Runs / Запуски тестів'); click('+ Add test run'); change('Name', 'Defect run'); change('Environment / Середовище', 'env-voicli-staging'); change('Build / Збірка', 'build-voicli-26-rc1'); change('Browser', 'Chrome'); change('Device / OS', 'macOS'); click('Select all visible'); click('Create Run'); click('TC-001') }
...
describe('Defects workspace', () => {
  it('searches, filters, creates and edits with Cancel, supports reopening Closed and project isolation', async () => {
    ...
    await project('QP Notes'); expect(rows()).toHaveLength(0); click('+ Add defect'); change('Title', 'QP bug'); click('Save Defect'); expect(within(defectPanel()).getByText('BUG-001')).toBeTruthy()
  ...
  it('keeps the existing-defect picker scoped to project and can link a completed Fail without editing history', async () => {
    await renderAuthenticatedApp(); await project('QP Notes'); click('Defects / Дефекти'); click('+ Add defect'); change('Title', 'Foreign QP bug'); click('Save Defect')
