...
describe('Defects workspace', () => {
  ...
  it('searches, filters, creates and edits with Cancel, supports reopening Closed and project isolation', async () => {
    ...
    await filter('Severity', 'Critical / Критичний'); await filter('Priority', 'Highest / Найвищий'); await filter('Status', 'Ready for Retest / Готовий до повторного тестування'); expect(rows()).toHaveLength(1)
    await filter('Priority', 'Low / Низький'); expect(rows()).toHaveLength(0); click('Clear filters')
    expect(screen.queryByRole('button', { name: 'Clear filters' })).toBeNull()
    await filter('Severity', 'Major / Значний'); expect(rows()).toHaveLength(1)
    await filter('Severity', 'Major / Значний'); expect(rows()).toHaveLength(3)
  ...
  it('creates from Fail, preserves context, navigates both directions and links several defects', async () => {
    ...
    click('Close execution'); fireEvent.keyDown(screen.getByRole('button', { name: 'Execution actions TC-001' }), { key: 'Enter' }); fireEvent.click(await screen.findByRole('menuitem', { name: 'View BUG-004 / Переглянути' }))
    expect(within(defectPanel()).getByText('Test Run: Defect run')).toBeTruthy(); expect(within(defectPanel()).getByText('Execution Result: Fail / Не пройдено')).toBeTruthy()
