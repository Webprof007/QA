...
describe('Run Reports UI', () => {
  it('renders Test Run report from existing results and returns to the selected execution', async () => {
    ...
    expect(report.getByText('Pass / Пройдено: 1')).toBeTruthy(); expect(report.getByText('Fail / Не пройдено: 1')).toBeTruthy()
  ...
  it('renders Smoke report with prerequisites, empty/not-run totals and source snapshot rows', async () => {
    ...
    expect(report.getByText('Total: 2')).toBeTruthy(); expect(report.getByText('Completed: 0')).toBeTruthy(); expect(report.getByText('Not Run / Не виконано: 2')).toBeTruthy(); expect(report.getByText('Not Checked / Не перевірено: 1')).toBeTruthy()
    ...
    expect(report.getByText('Fail / Не пройдено: 1')).toBeTruthy(); expect(report.getByText('Pass / Пройдено: 1')).toBeTruthy(); expect(report.getAllByText(/BUG-004/)).toHaveLength(2)
