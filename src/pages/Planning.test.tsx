...
describe('Test Plan', () => {
  it('creates, edits, cancels and retains a plan through navigation, scoped to project', async () => {
    ...
    click('+ Add test plan'); change('Title', 'Release validation'); change('Objective', 'Check critical flows'); change('Version', '2.0'); change('Status', 'Active'); change('Start date', '2026-09-14'); change('End date', '2026-09-15'); click('Save')
    ...
    await project('QP Notes'); expect(screen.queryByText('Release validation')).toBeNull(); click('+ Add test plan'); change('Title', 'QP plan'); click('Save')
  ...
  it('keeps multiple plans and edits only the selected plan', async () => {
    ...
    for (const title of ['Regression', 'Release 1.0', 'Release 2.0']) {
      click('+ Add test plan'); change('Title', title); change('Objective', `Objective for ${title}`); click('Save'); click('← Test Plans')
    }
    ...
    click('+ Add test plan'); change('Title', 'QP plan'); click('Save')
  ...
  it('cancels creation and validates schedule', async () => {
    await renderAuthenticatedApp(); click('Test Plan / План тестування'); click('+ Add test plan'); click('Cancel'); click('+ Add test plan')
...
describe('Checklist definitions and runs', () => {
  ...
  it('shares project Area creation and rename with Requirements and protects a used Area', async () => {
    ...
    click('Requirements / Вимоги'); click('+ Add requirement'); fireEvent.click(screen.getByRole('combobox', { name: 'Area' }))
    ...
    click('Checklists / Чеклісти'); click('Open Shared Area checklist'); expect(within(panel()).getByText('Shared preferences')).toBeTruthy()
    await project('QP Notes'); click('+ Add checklist'); fireEvent.click(screen.getByRole('combobox', { name: 'Area' })); expect(screen.queryByRole('button', { name: 'Shared preferences' })).toBeNull()
  })
})
