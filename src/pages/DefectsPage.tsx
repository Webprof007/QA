...
import { DefectStatusBadge } from '@/components/DomainBadge'
...
export function DefectsPage({ retests = [], cases = [], types = [], onRetest, onRetestTransition, setup = emptyProjectSetup, projectId, items, areas, runs, initialId, sourceRef, onSave }: { retests?: DefectRetest[]; cases?: TestCase[]; types?: TestCaseDictionaryValue[]; onRetest?: (id: string, input: RetestInput, attachments: RetestEvidenceAttachment[]) => Promise<RetestSaveResult>; onRetestTransition?: (id: string, action: RetestAction, retestId?: string) => Promise<string | null>; setup?: ProjectSetupState; projectId: string; items: Defect[]; areas: ProjectArea[]; runs: TestRunsState; initialId?: string; sourceRef?: DefectSourceRef; onSave: (draft: Defect) => Promise<Defect> }) {
  ...
  return <main className="smoke-app"><header className="page-heading"><h1>Defects</h1></header>
    ...
    </div><table className="tc-table defects-table"><thead><tr>{['ID', 'Title', 'Area', 'Severity', 'Priority', 'Status', 'Build / Збірка', 'Updated', 'Actions'].map(label => <th key={label}>{label in filterOptions ? filterHeader(label as keyof typeof filterOptions) : label}</th>)}</tr></thead><tbody>{visible.map(item => <tr key={item.id} className={selectedId === item.id ? 'tc-selected' : ''} onClick={() => openItem(item.id)}><td><button className="tc-open" aria-label={`Open ${item.code}`} onClick={() => openItem(item.id)}>{item.code}</button></td><td>{item.title}</td><td>{projectAreas.find(area => area.id === item.areaId)?.name || '—'}</td><td>{severityLabel(item.severity)}</td><td>{priorityLabel(item.priority)}</td><td><DefectStatusBadge value={item.status} compact /></td><td>{item.buildVersionSnapshot || '—'}</td><td>{new Date(item.updatedAt).toLocaleDateString()}</td><td><div className="table-row-actions" onClick={event => event.stopPropagation()}>{item.status === 'New' && onRetestTransition && <Button type="button" variant="outline" size="sm" aria-label={`Open Defect ${item.code}`} onClick={() => transition(item.id, 'Open')} title="Open Defect / Відкрити дефект">Open / Відкрити</Button>}{['Open', 'In Progress'].includes(item.status) && onRetestTransition && <Button type="button" variant="outline" size="sm" aria-label={`Mark Ready for Retest ${item.code}`} onClick={() => transition(item.id, 'Ready for Retest')} title="Mark Ready for Retest / Позначити готовим до повторного тестування">Ready / До ретесту</Button>}{item.status === 'Ready for Retest' && onRetest && <Button type="button" size="sm" aria-label={`Retest ${item.code}`} onClick={() => startRetest(item.id)} title="Retest / Повторне тестування">Retest / Ретест</Button>}<DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="ghost" size="icon" aria-label={`More actions ${item.code}`}><MoreHorizontal /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={() => openItem(item.id)}>View details / Переглянути</DropdownMenuItem><DropdownMenuItem onSelect={() => { setSelectedId(item.id); setRetestRequest(null); setDraft(structuredClone(item)); setError('') }}>Edit / Редагувати</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div></td></tr>)}</tbody></table>{!visible.length && <p className="muted">Дефектів за цими умовами немає.</p>}</div>
