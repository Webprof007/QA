...
describe('Project Settings and shared context UI', () => {
  ...
  it('creates/releases builds, preserves archived relationships and isolates setup and selectors by project', async () => {
    await renderAuthenticatedApp(); click('Settings / Налаштування'); click('Releases & Builds'); click('+ Add release'); change('Name', 'September'); change('Status', 'Active'); change('Start date', '2026-09-15'); click('Save')
    ...
    await project('QP Notes'); expect(screen.queryByRole('button', { name: 'Staging' })).toBeNull(); click('Releases & Builds'); expect(screen.queryByRole('button', { name: 'September rc' })).toBeNull()
  ...
  it('keeps Run and Defect incident snapshots through shared renames/deletions and supports explicit changes', async () => {
    ...
    click('Settings / Налаштування'); click('Staging'); change('Name', 'New staging'); click('Save'); click('Releases & Builds'); click('2.6.0-rc1'); change('Version', 'New version'); click('Save')
    ...
    click('Settings / Налаштування'); click('New staging'); click('Delete Environment'); click('Підтвердити видалення'); click('Releases & Builds'); click('New version'); click('Delete Build'); click('Підтвердити видалення')
  ...
  it('Smoke history renders shared snapshots even after environment/build deletion', async () => {
    ...
    click('Settings / Налаштування'); click('Staging'); click('Delete Environment'); click('Підтвердити видалення'); click('Releases & Builds'); click('2.6.0-rc1'); click('Delete Build'); click('Підтвердити видалення')
