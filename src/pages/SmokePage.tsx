...
type Props = { projectId: string; data: SmokeState; cases: TestCase[]; areas: ProjectArea[]; types: TestCaseDictionaryValue[]; userId?: number; onChange: Dispatch<SetStateAction<SmokeState>> }
export function SmokePage({ projectId, data, cases, areas, types, userId, onChange }: Props) {
  const [suiteId, setSuiteId] = useState(''), [runId, setRunId] = useState(''), [creatingRun, setCreatingRun] = useState(false)
  ...
  function create(input: SmokeRunInput) {
    try { const next = createSmokeRun(data, projectId, suiteId, input, casesInProject, projectAreas, projectTypes, userId); onChange(next); setRunId(next.runs[next.runs.length - 1].id); setCreatingRun(false); return null }
  ...
  return <main className="smoke-app"><header className="page-heading"><h1>Smoke</h1></header>
    {run ? <SmokeRunDetail key={run.id} run={run} data={data} userId={userId} onChange={onChange} onBack={() => setRunId('')} /> : <>
      ...
      <div className={`tc-layout ${editor || creatingRun ? 'tc-with-panel' : ''}`}><div className="tc-list">
        {!suite ? <><div className="tc-toolbar"><Button className="tc-add" onClick={() => setEditor({ id: crypto.randomUUID(), name: '', description: '', testCaseIds: [], prerequisites: [] })}>New Smoke Suite</Button></div>
          ...
          <section className="tc-section" aria-label="Smoke run history"><h3>Runs</h3><table className="tc-table"><thead><tr>{['Run', 'Environment / Середовище', 'Build / Збірка', 'Status', 'Progress', 'Started', 'Completed'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{history.map((item, index) => { const progress = runCounts(data.executions.filter(value => value.projectId === projectId && value.runId === item.id)); return <tr key={item.id} onClick={() => { setRunId(item.id); setEditor(null); setCreatingRun(false) }}><td><button className="tc-open" onClick={() => { setRunId(item.id); setEditor(null); setCreatingRun(false) }}>Run #{index + 1}</button></td><td>{item.environment || '—'}</td><td>{item.build || '—'}</td><td>{item.status}</td><td>{progress.done} / {progress.total}</td><td>{date(item.startedAt)}</td><td>{date(item.completedAt)}</td></tr> })}</tbody></table>{!history.length && <p className="muted">Запусків поки немає.</p>}</section>
        </div>}
      </div>{editor ? <SmokeSuiteEditor key={editor.id} initial={editor} code={suite?.code ?? nextSmokeCode(data.suites, projectId)} cases={casesInProject} areas={projectAreas} types={projectTypes} onSave={save} onCancel={() => setEditor(null)} /> : creatingRun && <SmokeRunCreate onCreate={create} onCancel={() => setCreatingRun(false)} />}</div>
    </>}
