...
describe('Defect Retest workflow', () => {
  it('supports manual Blocked → Fail → Reopen → Pass → Close with immutable numbered history', async () => {
    ...
    await screen.findByRole('button', { name: 'Retest #2' }); expect(defectPanel().getByText('Ready for Retest', { selector: 'p' })).toBeTruthy(); click('Reopen Defect'); expect(await defectPanel().findByText('Open', { selector: 'p' })).toBeTruthy(); expect(screen.queryByRole('button', { name: 'Retest' })).toBeNull()
    click('Mark Ready for Retest'); await defectPanel().findByText('Ready for Retest', { selector: 'p' }); retest('Pass'); change('Actual Result / Фактичний результат', 'Fix verified'); click('Save Retest'); await screen.findByRole('button', { name: 'Retest #3' }); expect(defectPanel().getByText('Ready for Retest', { selector: 'p' })).toBeTruthy(); click('Close Defect')
    expect(await defectPanel().findByText('Closed', { selector: 'p' })).toBeTruthy(); expect(screen.queryByRole('button', { name: 'Retest' })).toBeNull()
  ...
  it('uses current TestCase snapshot, defaults incident IDs and leaves incident build and original execution unchanged', async () => {
    await renderAuthenticatedApp(); click('Test Runs / Запуски тестів'); click('+ Add test run'); change('Name', 'Incident run'); change(env, 'env-voicli-staging'); change(build, 'build-voicli-26-rc1'); click('Select all visible'); click('Create Run'); await screen.findByText('Draft'); click('TC-001'); change('Result', 'Fail'); click('Save result'); await screen.findByRole('button', { name: 'Create Defect' }); click('Create Defect'); click('Save Defect'); const viewDefect = await screen.findByRole('button', { name: 'View Defect' }); fireEvent.click(viewDefect); await screen.findByRole('complementary', { name: 'Defect panel' }); click('Open Defect'); await defectPanel().findByText('Open', { selector: 'p' }); click('Mark Ready for Retest'); await defectPanel().findByText('Ready for Retest', { selector: 'p' })
    ...
    expect(screen.getByRole('heading', { name: 'TC-001 — Updated definition' })).toBeTruthy(); change(build, 'build-voicli-26-rc2'); change('Result', 'Pass'); click('Save Retest'); await screen.findByRole('button', { name: 'Retest #1' }); click('Close Defect'); await defectPanel().findByText('Closed', { selector: 'p' })
  ...
  it('keeps Retest unavailable for closed/rejected/duplicate and scopes history and selectors to project', async () => {
    await renderAuthenticatedApp(); click('Defects / Дефекти'); click('Open BUG-003'); retest('Pass'); click('Save Retest'); await screen.findByRole('button', { name: 'Retest #1' }); click('Close Defect'); await defectPanel().findByText('Closed', { selector: 'p' })
    for (const status of ['Rejected', 'Duplicate']) { click('Edit Defect'); change('Status', status); click('Save Defect'); await defectPanel().findByText(status, { selector: 'p' }); expect(screen.queryByRole('button', { name: 'Retest' })).toBeNull(); expect(screen.getByRole('button', { name: 'Retest #1' })).toBeTruthy() }
