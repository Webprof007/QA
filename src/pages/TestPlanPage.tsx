  ...
  return <main className="smoke-app"><header className="page-heading"><h1>Test Plan</h1></header>
    {!active ? <div className="tc-list">
      ...
      <table className="tc-table"><colgroup><col /><col style={{ width: '10%' }} /><col style={{ width: '13%' }} /><col style={{ width: '15%' }} /><col style={{ width: '15%' }} /><col style={{ width: '15%' }} /></colgroup>
        <thead><tr>{['Title / Назва', 'Version / Версія', 'Status / Статус', 'Start date / Дата початку', 'End date / Дата завершення', 'Updated / Оновлено'].map(label => <th key={label}>{label}</th>)}</tr></thead>
      ...
      <div className="tc-panel-actions"><AccountBackButton onClick={() => { setSelectedId(''); setDraft(null); setError('') }}>← Test Plans</AccountBackButton><Button type="button" variant="outline" size="sm" className="tc-add h-8" onClick={create}>+ Create Test Plan</Button></div>
