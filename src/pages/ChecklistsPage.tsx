...
export function ChecklistsPage({ projectId, items, runs, areas, onSave, onImport, onRunCreate, onRunItemSave, onRunComplete, onRun, onAreaSave, onAreaRemove }: Props) {
  ...
  return <DictionaryContext.Provider value={dictionary}><main className="smoke-app"><header className="page-heading"><h1>Checklists</h1></header>
    ...
    {run ? <ChecklistRunPanel run={run} onItemSave={item => onRunItemSave ? onRunItemSave(run.id, item) : Promise.resolve(onRun?.({ ...run, items: run.items.map(value => value.id === item.id ? item : value) }))} onComplete={() => onRunComplete ? onRunComplete(run.id) : Promise.resolve(onRun?.({ ...run, status: 'Completed', completedAt: new Date().toISOString() }))} onClose={() => { setRunId(''); setTab('Runs') }} /> : active && <aside className="tc-panel" aria-label="Checklist panel"><div className="panel-heading"><h2>{draft ? selected ? 'Edit Checklist' : 'New Checklist' : active.title}</h2><div className="tc-panel-actions">{!draft && tab === 'Definition' && <Button type="button" variant="outline" size="sm" onClick={() => setDraft(structuredClone(active))}>Edit Checklist</Button>}<Button variant="ghost" size="icon" aria-label="Close checklist" title="Close checklist" onClick={close}><X /></Button></div></div>
      ...
      {!draft && tab === 'Runs' ? <section aria-label="Run history"><table className="tc-table"><thead><tr>{['Run', 'Date', 'Status'].map(label => <th key={label}>{label}</th>)}{(['Pass', 'Fail', 'Blocked'] as const).map(result => <th key={result}>{resultLabel(result)}</th>)}</tr></thead><tbody>{history.map((entry, index) => <tr key={entry.id} onClick={() => setRunId(entry.id)}><td><button className="tc-open" onClick={() => setRunId(entry.id)}>Run {index + 1}</button></td><td>{new Date(entry.startedAt).toLocaleString()}</td><td>{entry.status}</td>{(['Pass', 'Fail', 'Blocked'] as const).map(result => <td key={result}><ResultBadge value={result} count={entry.items.filter(item => item.result === result).length} /></td>)}</tr>)}</tbody></table>{!history.length && <p>Проходжень поки немає.</p>}</section> : <form className="tc-form" onSubmit={save}>
