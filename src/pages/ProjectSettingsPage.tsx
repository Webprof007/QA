...
export function ProjectSettingsPage({ project, areas, types, data, onChange, onAreaSave, onAreaRemove, onTypeSave, onTypeRemove, onDeleteProject }: Props) {
  ...
  return <main className="smoke-app settings-page"><header className="page-heading"><h1>Settings / Налаштування</h1></header>
    <section className="settings-section" aria-labelledby="settings-general"><div className="settings-section-heading"><div><h2 id="settings-general">General</h2><p className="muted">Project details are read-only because the current backend does not provide a Project update endpoint.</p></div></div><dl className="settings-general"><div><dt>Project name</dt><dd>{project.name}</dd></div><div><dt>Description</dt><dd>{project.description || '—'}</dd></div></dl></section>
    <SettingsDictionarySection title="Project Areas" description="Shared by Requirements, Test Cases, Checklists, Defects, Audit and Coverage." entity="area" projectId={projectId} items={areas} onSave={onAreaSave} onRemove={onAreaRemove} />
    <SettingsDictionarySection title="Test Case Types" description="The shared type dictionary used by central Test Cases." entity="test case type" projectId={projectId} items={types} onSave={onTypeSave} onRemove={onTypeRemove} />
    <section className="settings-section" aria-labelledby="settings-environments"><div className="settings-section-heading"><div><h2 id="settings-environments">Environments</h2><p className="muted">Shared project environments. Frontend session only.</p></div><AddEntityButton entity="environment" onClick={() => create('Environment')} /></div><table className="tc-table" aria-label="Environments"><thead><tr>{['Name', 'Base URL', 'Status', 'Updated'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{environments.map(item => <tr key={item.id} onClick={() => { setError(''); setDraft({ kind: 'Environment', value: { ...item } }) }}><td><button className="tc-open">{item.name}</button></td><td>{item.baseUrl || '—'}</td><td>{item.isActive ? 'Active' : 'Inactive'}</td><td>{updated(item.updatedAt)}</td></tr>)}</tbody></table>{!environments.length && <p className="muted">Environments поки немає.</p>}</section>
    <section className="settings-section" aria-labelledby="settings-releases"><div className="settings-section-heading"><div><h2 id="settings-releases">Releases &amp; Builds</h2><p className="muted">Shared release/build context. Frontend session only.</p></div></div><SetupTable title="Releases" action={<AddEntityButton entity="release" onClick={() => create('Release')} />}><table className="tc-table" aria-label="Releases"><thead><tr>{['Release', 'Status', 'Start', 'Release date', 'Builds'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{releases.map(item => <tr key={item.id} onClick={() => { setError(''); setDraft({ kind: 'Release', value: { ...item } }) }}><td><button className="tc-open">{item.name}</button></td><td>{item.status}</td><td>{item.startDate || '—'}</td><td>{item.releaseDate || '—'}</td><td>{builds.filter(build => build.releaseId === item.id).length}</td></tr>)}</tbody></table>{!releases.length && <p className="muted">Releases поки немає.</p>}</SetupTable><SetupTable title="Builds" action={<AddEntityButton entity="build" onClick={() => create('Build')} />}><table className="tc-table" aria-label="Builds"><thead><tr>{['Version', 'Release', 'Description', 'Created'].map(label => <th key={label}>{label}</th>)}</tr></thead><tbody>{builds.map(item => <tr key={item.id} onClick={() => { setError(''); setDraft({ kind: 'Build', value: { ...item } }) }}><td><button className="tc-open">{item.version}</button></td><td>{releases.find(release => release.id === item.releaseId)?.name || '—'}</td><td>{item.description || '—'}</td><td>{updated(item.createdAt)}</td></tr>)}</tbody></table>{!builds.length && <p className="muted">Builds поки немає.</p>}</SetupTable></section>
    <section className="settings-section settings-danger" aria-labelledby="settings-danger"><div><h2 id="settings-danger">Danger Zone</h2><p className="muted">Permanently delete this Project and all project-owned QA data.</p></div><Button variant="destructive" onClick={onDeleteProject}>Delete Project</Button></section>
