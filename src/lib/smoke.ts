...
export type SmokeRunInput = Pick<SmokeRun, 'environmentId' | 'buildId' | 'browser' | 'deviceOrOs' | 'notes'>
...
export function createSmokeRun(state: SmokeState, projectId: string, suiteId: string, input: SmokeRunInput, cases: TestCase[], areas: ProjectArea[], types: TestCaseDictionaryValue[], userId?: number, setup: ProjectSetupState = emptyProjectSetup): SmokeState {
  ...
  const run: SmokeRun = { id, projectId, suiteId, suiteCodeSnapshot: suite.code, suiteNameSnapshot: suite.name, ...resolveProjectContext(setup, projectId, input), browser: input.browser, deviceOrOs: input.deviceOrOs, notes: input.notes, status: 'Draft', createdByUserId: userId, createdAt: now, updatedAt: now }
