...
function failedRun() {
  const state = createTestRun('voicli', { name: 'Regression', environmentId: 'env-voicli-staging', buildId: 'build-voicli-26-rc1', browser: 'Chrome', deviceOrOs: 'macOS', notes: '', testCaseIds: cases.items.slice(0, 2).map(item => item.id) }, cases.items, [], cases.areas, cases.types, [], createProjectSetupMockData())
...
describe('Defects model and execution links', () => {
  ...
  it('copies Fail context and origin, preserves own values after source edits', () => {
    ...
    expect(state.items[0]).toMatchObject({ source: { type: 'testExecution', id: runs.executions[0].id, runId: runs.runs[0].id }, sourceTestCaseId: runs.executions[0].testCaseId, title: 'Login valid user', environmentId: 'env-voicli-staging', environmentNameSnapshot: 'Staging', buildId: 'build-voicli-26-rc1', buildVersionSnapshot: '2.6.0-rc1', browser: 'Chrome', deviceOrOs: 'macOS', description: 'Initial context', actualResult: 'Actual output', evidenceNote: 'Evidence text', areaId: cases.items[0].areaId })
    ...
    runs.executions[0].testCaseSnapshot.title = 'Changed source'; runs.executions[0].testCaseSnapshot.steps[0].expectedResult = 'Changed expected'; runs.runs[0].buildVersionSnapshot = 'Different build'
    expect(state.items[0].title).toBe('Login valid user'); expect(state.items[0].buildVersionSnapshot).toBe('2.6.0-rc1'); expect(state.items[0].expectedResult).not.toContain('Changed expected')
  })
  it('supports multiple defects per execution and multiple executions per defect without duplicate links', () => {
    ...
    state = linkDefect(state, 'voicli', runs.executions[1].id, id, runs)
    state = linkDefect(state, 'voicli', runs.executions[1].id, id, runs)
    state = saveDefect(state, { ...newDefect('voicli', state.items), title: 'Second issue' }, 'voicli', 42, runs, cases.areas)
    state = linkDefect(state, 'voicli', runs.executions[0].id, state.items[1].id, runs)
  ...
  it('rejects cross-project and non-Fail links and unsafe URLs', () => {
    ...
    expect(() => linkDefect(state, 'voicli', runs.executions[0].id, state.items[0].id, runs)).toThrow()
