...
export function ExecutionPanel({ execution, draft, readOnly, onChange, onSave, onCancel, onClose, onPrevious, onNext, first, last, footer, evidence }: ExecutionPanelProps) {
  ...
  const staleActualResult = execution.result === 'Pass' && input.result === 'Fail' && Boolean(input.actualResult.trim())
  return <aside className="tc-panel" aria-label="Execution panel"><div className="panel-heading"><div><p className="test-id">{snapshot.code}</p><h2>{snapshot.title}</h2></div><Button variant="ghost" size="icon" aria-label="Close execution" title="Close execution" disabled={!!draft} onClick={onClose}><X /></Button></div>
    <div className="tc-form"><div className="tc-panel-actions"><Button variant="outline" disabled={first || !!draft} onClick={onPrevious}>Previous</Button><Button variant="outline" disabled={last || !!draft} onClick={onNext}>Next</Button></div>
      <dl className="tc-field-pair">{[['Area', snapshot.areaName], ['Priority', priorityLabel(snapshot.priority)], ['Type', snapshot.typeName], ['Status', snapshot.status]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value || '—'}</dd></div>)}</dl>
      ...
      <form className="tc-form" onSubmit={event => { event.preventDefault(); if (!readOnly) onSave() }}>
        <div className="field"><label htmlFor="execution-result">Result</label>{readOnly ? <ResultBadge value={execution.result} /> : <select id="execution-result" className="audit-select" value={input.result} onChange={event => onChange({ ...input, result: event.target.value as ExecutionView['result'] })}>{executionResults.map(value => <option key={value} value={value}>{resultLabel(value)}</option>)}</select>}</div>
        {staleActualResult && <p className="muted" role="status">Результат змінено з Pass на Fail. Перевірте та за потреби оновіть Actual Result перед збереженням.</p>}
        ...
        {!readOnly && <div className="tc-panel-actions"><Button type="submit" aria-label="Save result">Save result / Зберегти результат</Button>{draft && <Button type="button" variant="outline" onClick={onCancel}>Cancel changes</Button>}</div>}
