...
export function SmokeRunDetail({ run, data, userId, onChange, onBack, initialExecutionId }: { initialExecutionId?: string; run: SmokeRun; data: SmokeState; userId?: number; onChange: Dispatch<SetStateAction<SmokeState>>; onBack: () => void }) {
  const context = useDefectContext()
  const [selectedId, setSelectedId] = useState(initialExecutionId ?? ''), [draft, setDraft] = useState<SmokeExecutionInput | null>(null), [confirm, setConfirm] = useState(false), [reportOpen, setReportOpen] = useState(false)
