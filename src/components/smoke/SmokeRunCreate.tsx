...
export function SmokeRunCreate({ projectId, setup = emptyProjectSetup, onCreate, onCancel }: { projectId: string; setup?: ProjectSetupState; onCreate: (input: SmokeRunInput) => string | null; onCancel: () => void }) {
  ...
  return <aside className="tc-panel" aria-label="New Smoke Run"><div className="panel-heading"><h2>Run Smoke</h2><Button variant="ghost" size="icon" aria-label="Close run creation" title="Close run creation" onClick={onCancel}><X /></Button></div><form className="tc-form" onSubmit={event => { event.preventDefault(); setError(onCreate(input) ?? '') }}>
