...
export function SmokeRunCreate({ projectId, setup = emptyProjectSetup, onCreate, onCancel }: { projectId: string; setup?: ProjectSetupState; onCreate: (input: SmokeRunInput) => Promise<string | null>; onCancel: () => void }) {
  const [input, setInput] = useState<SmokeRunInput>({ environmentId: undefined, buildId: undefined, browser: '', deviceOrOs: '', notes: '' })
  const [error, setError] = useState('')
  return <aside className="tc-panel" aria-label="New Smoke Run"><div className="panel-heading"><h2>Run Smoke</h2><Button variant="ghost" size="icon" aria-label="Close run creation" title="Close run creation" onClick={onCancel}><X /></Button></div><form className="tc-form" onSubmit={event => { event.preventDefault(); void onCreate(input).then(message => setError(message ?? '')).catch(cause => setError(cause instanceof Error ? cause.message : 'Не вдалося створити Smoke Run.')) }}>
