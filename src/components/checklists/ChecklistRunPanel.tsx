...
export function ChecklistRunPanel({ run, onChange, onClose }: { run: ChecklistRun; onChange: (run: ChecklistRun) => void; onClose: () => void }) {
  ...
  return <aside className="tc-panel" aria-label="Checklist run"><div className="panel-heading"><h2>{run.titleSnapshot}</h2><Button variant="ghost" onClick={onClose}>Close run</Button></div>
