...
export function DefectSourceSummary({ defect, disabled }: { defect: Defect; disabled: boolean }) {
  ...
  return <section className="tc-section" aria-label="Source"><h3>Source</h3>
    {value.kind === 'auditFinding' ? <><p>Audit: {value.audit.code} — {value.audit.title}</p><p>Finding: {value.finding.code} — {value.finding.title}</p></> : <><p>{defect.source.type === 'smokeExecution' ? 'Smoke Run' : 'Test Run'}: {'name' in value.run ? value.run.name : value.run.suiteNameSnapshot}</p><p>Test Case: {value.execution.testCaseSnapshot.code} — {value.execution.testCaseSnapshot.title}</p><p>Execution Result: {value.execution.result}</p><p>Executed at: {value.execution.executedAt ? new Date(value.execution.executedAt).toLocaleString() : '—'}</p></>}
