...
export function DefectPanel({ retestSection, setup = emptyProjectSetup, item, editing, error, areas, sourceSection, onChange, onSave, onEdit, onCancel, onClose }: { retestSection?: ReactNode; setup?: ProjectSetupState; item: Defect; editing: boolean; error: string; areas: ProjectArea[]; sourceSection?: ReactNode; onChange: (item: Defect) => void; onSave: () => void; onEdit: () => void; onCancel: () => void; onClose: () => void }) {
  ...
  return <aside className="tc-panel" aria-label="Defect panel"><div className="panel-heading"><div><p className="test-id">{item.code}</p><h2>{editing ? 'Defect' : item.title}</h2></div><div className="tc-panel-actions">{!editing && <Button type="button" variant="outline" size="sm" aria-label="Edit Defect" onClick={onEdit}>Edit Defect / Редагувати дефект</Button>}<Button variant="ghost" size="icon" aria-label="Close defect" title="Close defect" onClick={onClose}><X /></Button></div></div>
