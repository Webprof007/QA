...
export function RetestContext({ value }: { value: Pick<DefectRetest, 'testCaseSnapshot' | 'defectContextSnapshot'> }) {
  ...
  return <section className="tc-section" aria-label="Retest context">{snapshot ? <>
    ...
    <dl className="tc-field-pair">{[['Area', snapshot.areaName], ['Priority', snapshot.priority], ['Type', snapshot.typeName], ['Status', snapshot.status]].map(([label, text]) => <div key={label}><dt>{label}</dt><dd>{text || '—'}</dd></div>)}</dl>
