...
export function DefectRetests({ smoke, defect, retests, setup, cases, runs, areas, types, onSave, onTransition }: DefectRetestsProps) {
  ...
  return <section className="tc-section" aria-label="Retest History"><h3>Retest History</h3>
    ...
    {draft && <form className="tc-form" aria-label="Retest form" onSubmit={event => { event.preventDefault(); if (!draft.result) { setError('Виберіть результат Retest.'); return } void onSave({ ...draft, result: draft.result }, attachments.items).then(message => { if (message) setError(message); else { setDraft(null); attachments.setItems([]); setError('') } }).catch(cause => setError(cause instanceof Error ? cause.message : 'Не вдалося зберегти Retest.')) }}>
