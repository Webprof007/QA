...
export function RequirementPanel({ item, testCases, mode, error, onChange, onSave, onEdit, onCancel, onClose }: Props) {
  ...
  return <aside className="tc-panel" aria-label="Requirement panel">
    ...
    <form className="tc-form" onSubmit={event => { event.preventDefault(); onSave() }}>
      ...
      </> : <dl className="tc-field-pair"><div><dt>Area</dt><dd>{dictionary.area.find(area => area.id === item.areaId)?.name || '—'}</dd></div><div><dt>Status</dt><dd>{requirementStatuses.find(option => option.value === item.status)?.label}</dd></div></dl>}
      ...
      <section className="tc-section" aria-label="Linked Test Cases"><h3>Linked Test Cases</h3>
        {linked.length ? <ul className="req-linked-list" aria-label="Selected test cases">{linked.map(test => <li key={test.id}><span><span className="test-id">{test.code}</span> {test.title}</span>{editing && <Button type="button" variant="ghost" size="icon" aria-label={`Unlink ${test.code}`} onClick={() => patch({ testCaseIds: item.testCaseIds.filter(id => id !== test.id) })}><X /></Button>}</li>)}</ul> : <p className="muted">No linked test cases.</p>}
        {editing && <Button type="button" variant="outline" size="sm" onClick={() => setPickerOpen(true)}>Manage linked test cases</Button>}
    ...
    {pickerOpen && <TestCaseLinkPicker testCases={available} selectedIds={item.testCaseIds} onApply={testCaseIds => { patch({ testCaseIds }); setPickerOpen(false) }} onClose={() => setPickerOpen(false)} />}
  </aside>
}
