...
export function RunReport({ kind, run, summary, onBack, onExecution, onDefect, testPlan, sourceSuite, prerequisites }: Props) {
  ...
  return <section className="run-report" aria-label={`${kind} report`}>
    ...
    <section className="tc-section" aria-label="Report summary"><h3>Summary</h3><div className="tc-toolbar"><strong>Total: {summary.total}</strong><span>Completed: {summary.done}</span><span>Remaining: {summary.remaining}</span>{Object.entries(summary.counts).map(([result, count]) => <ResultBadge key={result} value={result} count={count} />)}</div></section>
    {prerequisites && <section className="tc-section" aria-label="Prerequisites report"><h3>Prerequisites</h3><div className="tc-toolbar"><strong>Total: {prerequisites.total}</strong><span>Checked: {prerequisites.checked}</span><span>Remaining: {prerequisites.remaining}</span>{Object.entries(prerequisites.counts).map(([result, count]) => <ResultBadge key={result} value={result} count={count} />)}</div>{prerequisites.total > 0 && <table className="tc-table"><thead><tr><th>Prerequisite</th><th>Result</th></tr></thead><tbody>{prerequisites.items.map(item => <tr key={item.id}><td>{item.textSnapshot}</td><td><ResultBadge value={item.result} /></td></tr>)}</tbody></table>}</section>}
    ...
    <section className="tc-section"><h3>Executions</h3><table className="tc-table" aria-label="Report executions"><thead><tr><th>Test Case</th><th>Title</th><th>Result</th><th>Defects</th></tr></thead><tbody>{summary.executions.map(row => <tr key={row.id}><td><Button type="button" variant="link" onClick={() => onExecution(row.id)}>{row.testCaseSnapshot.code}</Button></td><td>{row.testCaseSnapshot.title}</td><td><ResultBadge value={row.result} /></td><td>{row.defects.length ? row.defects.map(defect => <Button key={defect.id} type="button" variant="link" onClick={() => onDefect(defect.id)}>{defect.code}</Button>) : '—'}</td></tr>)}</tbody></table>{!summary.total && <p className="muted">No executions in this Run.</p>}</section>
  </section>
}
