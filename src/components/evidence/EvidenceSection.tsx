...
export function EvidenceSection({ projectId, ownerType, ownerId, items, readOnly = false, temporary = false, onAddFile, onAddLink, onRemove }: Props) {
  ...
  const [linking, setLinking] = useState(false), [name, setName] = useState(''), [url, setUrl] = useState(''), [error, setError] = useState(''), [pending, setPending] = useState(false)
  const visible = items.filter(item => item.projectId === projectId && item.ownerType === ownerType && item.ownerId === ownerId)
  async function attempt(action: () => void | Promise<void>) { try { setPending(true); await action(); setError('') } catch (cause) { setError(cause instanceof Error ? cause.message : 'Не вдалося оновити вкладення.') } finally { setPending(false) } }
  return <section className="tc-section evidence-section" aria-label="Attachments / Evidence"><h3>Attachments / Evidence</h3>
    {!readOnly && <><div className="tc-panel-actions"><Button type="button" size="sm" variant="outline" disabled={pending} onClick={() => input.current?.click()}>Add file</Button><Button type="button" size="sm" variant="outline" disabled={pending} onClick={() => setLinking(true)}>Add link</Button><input hidden ref={input} type="file" multiple aria-label="Evidence files" onChange={event => { const files = Array.from(event.target.files ?? []); void attempt(async () => { for (const file of files) await onAddFile?.(file); event.target.value = '' }) }} /></div><p className="muted">{temporary ? 'Файли тимчасові до збереження запису. Frontend-ліміт: 20 MB на файл.' : 'Файли зберігаються у захищеному сховищі проєкту. Frontend-ліміт: 20 MB на файл.'}</p></>}
    ...
    {!visible.length ? <p className="muted">No attachments.</p> : <ul className="evidence-list">{visible.map(item => { const safe = safeEvidenceUrl(item.url, item.kind), image = item.kind === 'file' && item.mimeType?.startsWith('image/'), video = item.kind === 'file' && item.mimeType?.startsWith('video/'); return <li key={item.id}>
      {safe && image && <a href={safe} target="_blank" rel="noopener noreferrer" aria-label={`Preview ${item.name}`}><img className="evidence-thumbnail" src={safe} crossOrigin="use-credentials" alt={item.name} /></a>}
